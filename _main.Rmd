...
circular_tree <- gheatmap(circular_tree, newspecies_table, offset=0.15, width=0.05, colnames=FALSE) +
...
**PERMANOVA**
```{r q1_greenland, comment="", message=FALSE, warning=FALSE, eval=FALSE}
# mat <- genome_counts_filt %>%
#   column_to_rownames("genome") %>%
#   filter(rowSums(. != 0, na.rm = TRUE) > 0) %>%
#   select(where(~ !all(. == 0)))
# labs <- colnames(mat)
# 
# beta_q1n <- hillpair(mat, q = 1)
# attr(beta_q1n$S, "Labels") <- labs
# beta_q1n <-beta_q1n$S
# save(beta_q1n,file="output/beta_q1n_greenland.RData")
```
```{r permanova_greenland_q1_sled, comment="", message=FALSE, warning=FALSE}
# sample_metadata <- sample_metadata %>% 
#   filter(!str_starts(sample, "GL_02"))
# 
# load("output/beta_q1n_greenland.RData")
beta_q1n_mat <- as.matrix(beta_q1n$S)
...
remove_sled <- read_csv("resources/sample_metadata.csv") %>% 
  filter(season=="winter")
...
```{r load_genome_anno, warning=FALSE, comments="", message=FALSE}
genome_annotations <- read_tsv("resources/gene_annotations.tsv.xz") %>%
  mutate(genome = str_extract(gene, "^[^^]+")) %>%
  mutate(ec = str_c("[EC:",ec,"]")) %>% 
  select(genome, everything())
...
**Management: Civilian vs Military**
```{r}
sample_genome_size %>% 
  group_by(dog_group) %>%
  summarise(
  ...
  rename(Group = 1, "Genome lenght" = 2)
```
...
```{r size_per_sample_plot, warning=FALSE, comments="", message=FALSE, fig.height=3, fig.width=4, fig.fullwidth=TRUE}
sample_genome_size %>%
...
```{r amr_per_group_plot, warning=FALSE, comments="", message=FALSE, fig.height=3, fig.width=4, fig.fullwidth=TRUE}
sample_amr %>% 
...
sample_vf %>% 
  ...
  rename("Sled dog" = 1, "Virulence factor" = 2)
```
...
**Phylogenetic**
```{r permanova_q1p_filt_season, comment="", message=FALSE, warning=FALSE, eval=FALSE}
beta_q1p_mat <- as.matrix(beta_q1p$S)
