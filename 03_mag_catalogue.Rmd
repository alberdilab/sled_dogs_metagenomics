...
# Add phylum ring
circular_tree <- gheatmap(circular_tree, phylum_heatmap, offset=-0.1, width=0.1, colnames=FALSE) +
...
circular_tree <-  circular_tree +
        ...
        annotate('text', x=3.825, y=0, label='                         Genome size', family='arial', size=3.5)

# note to self: use the number of spaces in label= to shift the y position of the text relative to the rings. 
...
# Some annotation
genome_annotations %>%
  filter(if_any(c(kegg_hit, pfam_hits, cazy_hits), ~ !is.na(.))) %>%
  nrow()
```
```{r}
# KEGG annotation
